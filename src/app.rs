...
impl CustomerApp {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let db_path = PathBuf::from("customer.db");
        ...
        egui::SidePanel::left("form_panel").resizable(false).default_width(280.0).show(ctx, |ui| {
            ...
            ui.group(|ui| {
                ui.vertical_centered(|ui| {
                    ui.heading(if self.is_editing { "📝 編輯客戶" } else { "➕ 新增客戶" });
                });
                ui.add_space(10.0);
                ui.label("姓名："); ui.text_edit_singleline(&mut self.new_name); ui.add_space(5.0);
                ui.label("電子郵件："); ui.text_edit_singleline(&mut self.new_email); ui.add_space(5.0);
                ui.label("聯絡電話："); ui.text_edit_singleline(&mut self.new_phone); ui.add_space(5.0);
        ...
        egui::TopBottomPanel::bottom("footer").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if let Some(_id) = self.selected_customer_id {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.button("🗑️ 刪除選取項目").clicked() { self.delete_customer(); }
                    });
                }
            });
            ui.add_space(5.0);
                    ...
                    frame.show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ui.vertical(|ui| {
                                ...
                                ui.label(format!("📍 地址：{}", customer.address));
                            });
                            
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                // 重要：將編輯按鈕放在最前面，並明確捕獲點擊
                                if ui.button("✏️ 編輯").clicked() {
                                    self.selected_customer_id = Some(id);
                                    self.new_name = customer.name.clone();
                                    self.new_email = customer.email.clone();
                                    self.new_phone = customer.phone.clone();
                                    self.new_address = customer.address.clone();
                                    self.is_editing = true;
                                    self.show_status(&format!("正在編輯：{}", customer.name));
                                    ctx.request_repaint();
                                }
                                ...
                                if ui.interact(card_rect, ui.id().with(id), egui::Sense::click()).clicked() {
                                    if !self.is_editing {
                                        self.selected_customer_id = Some(id);
                                        self.show_status(&format!("已選取：{}", customer.name));
                                    }
                                }
                            });
                        });
